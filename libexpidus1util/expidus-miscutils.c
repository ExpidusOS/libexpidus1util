...
static gchar *expidus_homedir = NULL; /* path to users home directory */
static gchar *expidus_userdir = NULL; /* path to users .expidus1 directory */
...
internal_initialize(void)
{
  ...
  if (dir != NULL)
    ...
    {
      expidus_userdir = g_build_filename (expidus_homedir, XFCE4DIR, NULL);
    }
}
 ...
 */
const gchar*
expidus_version_string (void)
{
  return EXPIDUS_VERSION_STRING;
}
 ...
 *
 * The returned string is owned by libexpidus1util and must not be freed by
 * the caller.
 *
 * Return value: the path to the current user's home directory.
 **/
const gchar*
expidus_get_homedir (void)
{
  G_LOCK (_lock);
  if (!expidus_homedir)
    internal_initialize ();
  ...
  return expidus_homedir;
}



/**
 * expidus_get_homefile_r:
 * @buffer  : pointer to a user provided destination buffer.
 * @length  : size of @buffer in bytes.
 * @format  : printf style format string.
 * @...     : the arguments to substitute in the output.
 *
 * Similar in functionality to #expidus_get_homefile, but uses a user
 * defined @buffer instead of allocating a new buffer.
 *
 * expidus_get_homefile_r uses safe string operations, that says, it garanties
 ...
 **/
gchar*
expidus_get_homefile_r (gchar *buffer, size_t len, const gchar *format, ...)
{
  gchar  *ptr;
  va_list ap;

  va_start (ap, format);
  ptr = internal_get_file_r (expidus_get_homedir (), buffer, len, format, ap);
  va_end (ap);
 ...
 *
 * The returned string is managed by libexpidus1util and must not be freed by
 * the caller.
 *
 * Return value: the path to the current user's ".expidus1" directory.
 */
const gchar*
expidus_get_userdir (void)
{
  G_LOCK(_lock);
  if (!expidus_userdir)
    {
      ...
      if (!g_file_test (expidus_userdir, G_FILE_TEST_IS_DIR))
        expidus_mkdirhier (expidus_userdir, 0700, NULL);
    }
  G_UNLOCK(_lock);

  return expidus_userdir;
}



/**
 * expidus_get_userfile_r:
 ...
 **/
gchar*
expidus_get_userfile_r (gchar *buffer, size_t length, const gchar *format, ...)
{
  gchar  *ptr;
  va_list ap;

  va_start (ap, format);
  ptr = internal_get_file_r (expidus_get_userdir (), buffer, length, format, ap);
  va_end (ap);
...
expidus_is_valid_tilde_prefix (const gchar *p)
{
 ...
 **/
gchar *
expidus_expand_variables (const gchar *command,
                       gchar      **envp)
{
  GString        *buf;
  const gchar    *start;
  ...
  for (p = command; *p != '\0'; ++p)
    {
      ...
      if (*p == '~'
          && (p == command
              || expidus_is_valid_tilde_prefix (p - 1)))
        {
          ...
          if (G_LIKELY (start == p))
            {
              ...
              buf = g_string_append (buf, expidus_get_homedir ());
            }
          else
            {
#ifdef HAVE_GETPWNAM
...
void
expidus_append_quoted (GString     *string,
...
/**
 * expidus_expand_desktop_entry_field_codes:
 ...
 **/
gchar*
expidus_expand_desktop_entry_field_codes (const gchar *command,
                                       GSList      *uri_list,
                                       const gchar *icon,
                                       const gchar *name,
                                       const gchar *uri,
      ...
      if (G_UNLIKELY (p[0] == '%' && p[1] != '\0'))
        {
          switch (*++p)
            ...
            case 'U':
              for (li = uri_list; li != NULL; li = li->next)
                {
                  expidus_append_quoted (string, li->data);
            ...
            case 'i':
              if (! expidus_str_is_empty (icon))
                {
                  g_string_append (string, "--icon ");
                  expidus_append_quoted (string, icon);
                }
              break;

            case 'c':
              if (! expidus_str_is_empty (name))
                expidus_append_quoted (string, name);
              break;

            case 'k':
              if (! expidus_str_is_empty (uri))
                expidus_append_quoted (string, uri);
              break;
...
#define __EXPIDUS_MISCUTILS_C__
