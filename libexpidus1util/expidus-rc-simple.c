                     ...
                     gboolean      readonly)
{
  ...
  /* attach callbacks */
  simple->__parent__.close        = _expidus_rc_simple_close;
  simple->__parent__.get_groups   = _expidus_rc_simple_get_groups;
  simple->__parent__.get_entries  = _expidus_rc_simple_get_entries;
  simple->__parent__.delete_group = _expidus_rc_simple_delete_group;
  simple->__parent__.get_group    = _expidus_rc_simple_get_group;
  simple->__parent__.has_group    = _expidus_rc_simple_has_group;
  simple->__parent__.set_group    = _expidus_rc_simple_set_group;
  simple->__parent__.delete_entry = _expidus_rc_simple_delete_entry;
  simple->__parent__.has_entry    = _expidus_rc_simple_has_entry;
  simple->__parent__.read_entry   = _expidus_rc_simple_read_entry;

  if (!readonly)
    {
      simple->__parent__.flush       = _expidus_rc_simple_flush;
      simple->__parent__.rollback    = _expidus_rc_simple_rollback;
      simple->__parent__.is_dirty    = _expidus_rc_simple_is_dirty;
      simple->__parent__.is_readonly = _expidus_rc_simple_is_readonly;
      simple->__parent__.write_entry = _expidus_rc_simple_write_entry;
    }
...
_expidus_rc_simple_parse (ExpidusRcSimple *simple)
{
  ...
  _expidus_return_val_if_fail (simple->filename != NULL, FALSE);

  rc = EXPIDUS_RC (simple);
  readonly = expidus_rc_is_readonly (rc);
  ...
  while (fgets (line, LINE_MAX, fp) != NULL)
    {
      ...
      if (!readonly || expidus_locale_match (rc->locale, locale) > EXPIDUS_LOCALE_NO_MATCH)
  ...
  if (G_LIKELY (entry != NULL))
    {
      ...
      if (G_LIKELY (translated && rc->locale != NULL))
        {
          best_match = EXPIDUS_LOCALE_NO_MATCH;
          best_value = NULL;

          for (lentry = entry->lfirst; lentry != NULL; lentry = lentry->next)
            {
              match = expidus_locale_match (rc->locale, lentry->locale);
              if (match == EXPIDUS_LOCALE_FULL_MATCH)
                {
                  /* FULL MATCH */
                  return lentry->value;
                }
...
#define __EXPIDUS_RC_SIMPLE_C__
