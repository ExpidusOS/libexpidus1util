...
_expidus_rc_init (XfceRc *rc)
{
  ...
  _expidus_return_if_fail (rc != NULL);
 ...
 * you pass %TRUE for @readonly, #expidus_rc_simple_open will fail if @filename
                     ...
                     gboolean     readonly)
{
  ...
  simple = _expidus_rc_simple_new (NULL, filename, readonly);

  if (exists && !_expidus_rc_simple_parse (simple))
    {
      expidus_rc_close (EXPIDUS_RC (simple));
      return NULL;
    }

  return EXPIDUS_RC (simple);
}



/**
 * expidus_rc_config_open:
 ...
 * If @readonly is %TRUE parsing is generally faster, because only untranslated
 * entries and entries that match the current locale will be loaded. Also if
 * you pass %TRUE for @readonly, #expidus_rc_config will fail if @resource
 * does not reference a regular file.
 *
 ...
 * dirty entries will be written at a subsequent #expidus_rc_flush call.
 *
 * Since: 4.2
 **/
void
 ...
 *
 * %NULL is a valid input value for @group. #expidus_rc_get_entries will
 ...
 * therefore all calls to #expidus_rc_read_entry and related functions will return
 * the fallback values. If @global is %FALSE, the @group will be deleted in the
 * per-user config file, and further calls to #expidus_rc_read_entry will most
 ...
 * to #expidus_rc_read_entry and #expidus_rc_write_entry will be applied only in the
 * active group.
 *
 ...
 *
 * Similar to #expidus_rc_delete_group, but works on an entry in the current
 * group.
 *
 * Since: 4.2
 **/
                         ...
                         gboolean      fallback)
{
  ...
  value = expidus_rc_read_entry (rc, key, NULL);
  if (value != NULL)
    {
 ...
 * on the next call to #expidus_rc_flush or when @rc is destroyed using
 * #expidus_rc_close.
 *
 * If @rc was opened using #expidus_rc_config_open, the value will be
 ...
 * @value : the value to write.
 *
 * Wrapper for #expidus_rc_write_entry, that stores a boolean @value.
 *
 * Since: 4.2
 **/
void
expidus_rc_write_bool_entry (XfceRc      *rc,
                          ...
                          gboolean     value)
{
  expidus_rc_write_entry (rc, key, value ? "true" : "false");
}
 ...
 * @value : the value to write.
 *
 * Wrapper for #expidus_rc_write_entry, that stores an integer @value.
 *
 * Since: 4.2
 **/
void
                         ...
                         gint         value)
{
  ...
  expidus_rc_write_entry (rc, key, buffer);
}
 ...
 *
 * Wrapper for #expidus_rc_write_entry, that stores a string list @value.
 *
 * Since: 4.2
 **/
void
                          ...
                          const gchar *separator)
{
  ...
  expidus_rc_write_entry (rc, key, list);
  g_free (list);
}



#define __EXPIDUS_RC_C__
