...
/**
 * SECTION: expidus-posix-signal-handler
...
static gboolean
expidus_posix_signal_handler_pipe_io(GIOChannel *source,
...
static void
expidus_posix_signal_handler(gint signal_id)
{
...
/**
 * expidus_posix_signal_handler_init:
 ...
 **/
gboolean
expidus_posix_signal_handler_init(GError **error)
{
    ...
    __handlers = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
                                       (GDestroyNotify)expidus_posix_signal_handler_data_free);
    ...
    __io_watch_id = g_io_add_watch(__signal_io, G_IO_IN,
                                   expidus_posix_signal_handler_pipe_io, NULL);
...
/**
 * expidus_posix_signal_handler_set_handler:
                                      ...
                                      GError **error)
{
    ...
    if(G_UNLIKELY(!__inited)) {
        if(error) {
            g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                        _("expidus_posix_signal_handler_init() must be called first"));
        }
        return FALSE;
    }
    ...
    sa.sa_handler = expidus_posix_signal_handler;
...
/**
 * expidus_posix_signal_handler_restore_handler:
 * @signal: A POSIX signal id number.
 *
 * Restores the default handler for @signal.
 **/
void
expidus_posix_signal_handler_restore_handler(gint signal)
{
    if(G_UNLIKELY(!__inited))
        return;
...
#define __EXPIDUS_POSIX_SIGNAL_HANDLER_C__
