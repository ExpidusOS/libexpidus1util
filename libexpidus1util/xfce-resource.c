...
#define TYPE_VALID(t) ((gint)(t) >= EXPIDUS_RESOURCE_DATA && (t) <= EXPIDUS_RESOURCE_THEMES)
  ...
  if (result == NULL)
    {
      if (*fallback == '~')
        {
          g_strlcpy (buffer, expidus_get_homedir(), PATH_MAX);
...
_res_init (void)
{
  ...
  _save[EXPIDUS_RESOURCE_CACHE] = g_strdup (dir);
  _list[EXPIDUS_RESOURCE_CACHE] = g_slist_prepend (_list[EXPIDUS_RESOURCE_CACHE], g_strdup (dir));
  ...
  _save[EXPIDUS_RESOURCE_DATA] = g_strdup (dir);
  _list[EXPIDUS_RESOURCE_DATA] = g_slist_prepend (_list[EXPIDUS_RESOURCE_DATA], g_strdup (dir));
  ...
  _save[EXPIDUS_RESOURCE_CONFIG] = g_strdup (dir);
  _list[EXPIDUS_RESOURCE_CONFIG] = g_slist_prepend (_list[EXPIDUS_RESOURCE_CONFIG], g_strdup (dir));

  /*
   * Data dirs
   */
  dirs = _res_getenv ("XDG_DATA_DIRS", DEFAULT_XDG_DATA_DIRS);
  _res_split_and_append (dirs, EXPIDUS_RESOURCE_DATA);
  _res_split_and_append (DATADIR, EXPIDUS_RESOURCE_DATA);

  /*
   * Config dirs
   */
  dirs = _res_getenv ("XDG_CONFIG_DIRS", DEFAULT_XDG_CONFIG_DIRS);
  _res_split_and_append (dirs, EXPIDUS_RESOURCE_CONFIG);
  _res_split_and_append (SYSCONFDIR "/xdg", EXPIDUS_RESOURCE_CONFIG);

  /*
   * Icons dirs
   */
  /* local icons dir first */
  path = g_build_filename (_save[EXPIDUS_RESOURCE_DATA], "icons", NULL);
  _save[EXPIDUS_RESOURCE_ICONS] = g_strdup (path);
  _list[EXPIDUS_RESOURCE_ICONS] = g_slist_prepend (_list[EXPIDUS_RESOURCE_ICONS], path);

  /* backward compatibility */
  path = expidus_get_homefile (".icons", NULL);
  _list[EXPIDUS_RESOURCE_ICONS] = g_slist_append (_list[EXPIDUS_RESOURCE_ICONS], path);

  for (l = _list[EXPIDUS_RESOURCE_DATA]; l != NULL; l = l->next)
    {
  ...
  /* XDG fallback */
  _list[EXPIDUS_RESOURCE_ICONS] = g_slist_append (_list[EXPIDUS_RESOURCE_ICONS], "/usr/share/pixmaps");

  /* fallback for system which that don't install everything in /usr */
  _list[EXPIDUS_RESOURCE_ICONS] = g_slist_append (_list[EXPIDUS_RESOURCE_ICONS], "/usr/local/share/pixmaps");
  _list[EXPIDUS_RESOURCE_ICONS] = g_slist_append (_list[EXPIDUS_RESOURCE_ICONS], DATADIR "/share/pixmaps");

  /*
   * Themes dirs
   */
  path = expidus_get_homefile (".themes", NULL);
  _save[EXPIDUS_RESOURCE_THEMES] = g_strdup (path);
  _list[EXPIDUS_RESOURCE_THEMES] = g_slist_prepend (_list[EXPIDUS_RESOURCE_THEMES], path);

  for (l = _list[EXPIDUS_RESOURCE_DATA]; l != NULL; l = l->next)
    {
      path = g_build_filename ((const gchar *) l->data, "themes", NULL);
      _list[EXPIDUS_RESOURCE_THEMES] = g_slist_append (_list[EXPIDUS_RESOURCE_THEMES], path);
    }

  /* Remove trailing slashes */
#define REMOVE_TRAILING_SLASHES(type) { _list[(type)] = _res_remove_trailing_slashes (_list[(type)]); }
  REMOVE_TRAILING_SLASHES (EXPIDUS_RESOURCE_DATA);
  REMOVE_TRAILING_SLASHES (EXPIDUS_RESOURCE_CONFIG);
  REMOVE_TRAILING_SLASHES (EXPIDUS_RESOURCE_CACHE);
  REMOVE_TRAILING_SLASHES (EXPIDUS_RESOURCE_ICONS);
  REMOVE_TRAILING_SLASHES (EXPIDUS_RESOURCE_THEMES);
#undef REMOVE_TRAILING_SLASHES

  /* remove duplicates from the lists */
#define REMOVE_DUPLICATES(type) { _list[(type)] = _res_remove_duplicates (_list[(type)]); }
  REMOVE_DUPLICATES (EXPIDUS_RESOURCE_DATA);
  REMOVE_DUPLICATES (EXPIDUS_RESOURCE_CONFIG);
  REMOVE_DUPLICATES (EXPIDUS_RESOURCE_CACHE);
  REMOVE_DUPLICATES (EXPIDUS_RESOURCE_ICONS);
  REMOVE_DUPLICATES (EXPIDUS_RESOURCE_THEMES);
 ...
 * expidus_resource_lookup(), expidus_resource_lookup_dirs() or
 * expidus_resource_match() instead.
 *
 ...
 *             a file.
 *
 * Similar to expidus_resource_lookup(), but returns all resource of the specified @type,
 ...
 * specified by @type.
 *
 * If @pattern contains a trailing slash, #expidus_resource_match looks only for
 ...
 * functions like #expidus_resource_lookup or #expidus_resource_save_location.
 *
 * Example: expidus_resource_match (EXPIDUS_RESOURCE_CONFIG, "foo/bar*") will probably
 ...
 * in your MCS, you should call expidus_resource_push_path() prior to calling
 * one of the resource search functions and call expidus_resource_pop_path()
 * right afterwards.
 *
 * Since: 4.2
 **/
...
/**
 * expidus_resource_pop_path:
 * @type : type of the resource which search list should be shrinked.
 *
 * Undoes the effect of the latest call to expidus_resource_push_path(). You
 * should take special care to call expidus_resource_pop_path() exactly same
 * times as expidus_resource_push_path(), everything else might result in
                             ...
                             gboolean         create)
{
  ...
  if (relpath[strlen (relpath) - 1] == G_DIR_SEPARATOR)
    {
      if (create && !expidus_mkdirhier (path, 0700, NULL))
  {
    g_free (path);
    path = NULL;
  }
  ...
  else
    {
      ...
      if (create && !expidus_mkdirhier (dir, 0700, NULL))
        {
          g_free (path);
          path = NULL;
        }
