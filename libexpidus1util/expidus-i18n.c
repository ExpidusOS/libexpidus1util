 ...
 * Copyright (c) 2004      Jasper Huijsman <jasper@expidus.org>
 * All rights reserved.
 *
...
static gchar *expidus_localize_path_internal (gchar       *buffer,
                                           gsize        len,
                                           const gchar *path,
                                           GFileTest    test);
...
static gchar*
expidus_localize_path_internal (gchar       *buffer,
                             gsize        len,
                             const gchar *path,
                             GFileTest    test)
{
 ...
 **/
void
expidus_textdomain (const gchar *package,
 ...
 **/
gchar*
expidus_get_file_localized (const gchar *filename)
{
  gchar buffer[PATH_MAX + 1];

  g_return_val_if_fail (filename != NULL, NULL);

  return g_strdup (expidus_localize_path_internal (buffer, sizeof (buffer), filename, G_FILE_TEST_IS_REGULAR));
}



/**
 * expidus_get_file_localized_r:
 * @buffer   : destination buffer to store the localized filename to.
 * @length   : size of @buffer in bytes.
 * @filename : name of a file to look for a localized version.
 *
 * Similar in functionality to expidus_get_file_localized(), but stores the
 * result in @buffer instead of allocating a new buffer.
 *
 * Return value: pointer to @buffer or %NULL on error.
 **/
gchar*
expidus_get_file_localized_r (gchar *buffer, gsize length, const gchar *filename)
{
  g_return_val_if_fail (buffer != NULL, NULL);
  g_return_val_if_fail (filename != NULL, NULL);

  return expidus_localize_path_internal (buffer, length, filename, G_FILE_TEST_IS_REGULAR);
}



/**
 * expidus_get_dir_localized:
 * @directory : directory name to check for a localized variant.
 *
 * Similar to expidus_get_file_localized(), but works on directory instead of
 * a file.
 *
 ...
 **/
gchar*
expidus_get_dir_localized (const gchar *directory)
{
  gchar buffer[PATH_MAX + 1];

  g_return_val_if_fail (directory != NULL, NULL);

  return g_strdup (expidus_localize_path_internal (buffer, sizeof (buffer), directory, G_FILE_TEST_IS_DIR));
}



/**
 * expidus_get_dir_localized_r:
 * @buffer    : destination buffer to store the localized filename to.
 * @length    : size of @buffer in bytes.
 * @directory : name of directory to check for localized variant of.
 *
 * Similar to #expidus_get_file_localized_r, but works on directory instead
 * of regular file.
 *
 * Return value: pointer to @buffer or %NULL on error.
 **/
gchar*
expidus_get_dir_localized_r (gchar *buffer, gsize length, const gchar *directory)
{
  g_return_val_if_fail (buffer != NULL, NULL);
  g_return_val_if_fail (directory != NULL, NULL);

  return expidus_localize_path_internal (buffer, length, directory, G_FILE_TEST_IS_DIR);
}



/**
 * expidus_get_path_localized:
 ...
 */
gchar*
expidus_get_path_localized (gchar       *dst,
                         gsize        size,
                         const gchar *paths,
 ...
 *               the constant #EXPIDUS_LOCALE_FULL_MATCH indicates a full match
 *               and #EXPIDUS_LOCALE_NO_MATCH means no match. Every other value
 *               indicates a partial match, the higher the value, the better
 *               the match. You should not rely on any specific value besides
 *               the constants #EXPIDUS_LOCALE_FULL_MATCH and #EXPIDUS_LOCALE_NO_MATCH,
 *               since the range of returned values may change in the future.
 *
 * Since: 4.2
 **/
guint
expidus_locale_match (const gchar *locale1,
                   const gchar *locale2)
{
  g_return_val_if_fail (locale1 != NULL, EXPIDUS_LOCALE_NO_MATCH);
  g_return_val_if_fail (locale2 != NULL, EXPIDUS_LOCALE_NO_MATCH);
  ...
  if (*locale1 == '\0')
    {
      if (*locale2 == '\0')
        return EXPIDUS_LOCALE_FULL_MATCH;
  ...
  else if (*locale2 == '\0')
    {
      switch (*locale1)
        {
          case '@': return EXPIDUS_LOCALE_NO_MATCH + 3;
          case '.': return EXPIDUS_LOCALE_NO_MATCH + 2;
          case '_': return EXPIDUS_LOCALE_NO_MATCH + 1;
        }

      /* FALL-THROUGH */
    }

  return EXPIDUS_LOCALE_NO_MATCH;
}



#define __EXPIDUS_I18N_C__
